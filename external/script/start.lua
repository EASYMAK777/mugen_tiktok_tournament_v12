...
function start.f_remapAI(ai)
	-- sanitize ai argument (must be a number or nil)
	if type(ai) == 'table' then
		ai = ai[1] or ai[2] or ai.ai
	end
...
function launchFight(data)
	...
	while true do
		...
		elseif winnerteam() ~= 1 and main.elimination then
			setMatchNo(-1)
			break
		-- bossrush: retry the same boss on loss (no continues)
		elseif winnerteam() ~= 1 and (gamemode('bossrush') or gamemode('bossrushcoop')) then
			setContinue(false)
			loopCount = loopCount + 1
			-- no break: restart fight loop
		-- player won or continuing is disabled
		elseif winnerteam() == 1 or (not t.continue and not (gamemode('bossrush') or gamemode('bossrushcoop'))) then
			start.p[2].t_selected = {}
			start.p[2].t_selTemp = {}
			setMatchNo(matchno() + 1)
			setContinue(false)
