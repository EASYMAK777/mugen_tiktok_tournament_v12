...
vec3 Horz3(vec2 pos, float off) {
    ...
    float scale = hardPix;  // remove * 0.01 for NO BLUR
    float wb = Gaus(dst - 1.1, scale);
    float wc = Gaus(dst + 0.0, scale);
    float wd = Gaus(dst + 1.1, scale);
...
vec3 Mask(vec2 pos) {
    ...
    pos.y = fract(pos.y / 6.0);
    if (pos.y < 0.333) mask.r = maskLight;
